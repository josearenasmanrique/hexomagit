...
try {
    ...
    AND ag.mes = $mesNumero AND ag.año = $anoActual
    ...
    // Consulta para obtener la suma de montos con estatus de pago "Completado" o "Finalizado"
    $sqlSumaMontos = "
    SELECT SUM(pg.Monto) AS MontoTotal
    FROM agenda AS ag
    JOIN pagos AS pg ON pg.idAgenda = ag.idAgenda
    WHERE ag.mes = $mesNumero AND ag.año = $anoActual
    AND (pg.Estatus_Pago = 'Completado' OR pg.Estatus_Pago = 'Finalizado');
    ";
    $stmtSumaMontos = $conn->query($sqlSumaMontos);
    $sumaMontos = $stmtSumaMontos->fetch(PDO::FETCH_ASSOC);
...
<h2>Total de Montos Pagados: $<?php echo $sumaMontos['MontoTotal']; ?></h2>
<br>
